...
import {
  ...
  createCategory,  // Keep if needed for creation
  updateCategory, 
  updatecategoryStatus, // Keep if needed for updates
  deleteCategory   // Keep if needed for deletion
} from '../../../features/servicenow/product-offering/productOfferingCategorySlice'; // Adjust path
import Table from '../../../utils/table/Table';
import ProductOfferingCategoryForm from '../../../components/servicenow/product offering/ProductOfferingCategoryForm';
...
function ProductOfferingCategory() {
  ...
  const handleUpdate = (id, updatedData) => dispatch(updateCategory({ id, ...updatedData }));
  const handleUpdateStatus = (id, currentStatus) => {
    ...
    if (window.confirm(`Are you sure you want to ${newStatus} this category?`)) {
      dispatch(updatecategoryStatus({ id, status: newStatus })) // Corrected action and payload
        .unwrap()
        .then(() => {
          alert(`Category ${newStatus}!`);
          dispatch(getall()); // Refresh the list after status change
        })
        .catch((err) => {
  ...
  // --- Form Submission Handler (called by AddCategoryForm) ---
  const handleCreateSubmit = (categoryData) => {
    ...
    // Handler for submitting the UPDATE form
      const handleUpdateSubmit = (editedData) => {
          ...
          return dispatch(updateCategory({ id: idToUpdate, ...editedData })) // Dispatch update action
              .unwrap()
              .then(() => {
  ...
  if(products.length > 0){
    products.map((product) => (colBodyContent.push(<tr key={product.sys_id} id={product.sys_id}>
      ...
      <Button className="mx-1" cyan onClick={() => handleOpenUpdateModal(product.sys_id)}>Update</Button>
      <Popconfirm
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h1>Product Offering Category</h1>
      ...
      */}</div>
        ...
        !loading && <div style={{ marginTop: '20px', fontStyle: 'italic' }}>No Categories found.</div>
      )}
